...
public class AudioRecorder {
     ...
     */
    public void startRecord(Context context, final AudioRecordStreamListener listener) {
        ...
        try {
            if (fileNameList.size() > 0) {
                ...
                for (String fileName : fileNameList) {
                    filePaths.add(AudioFileUtil.getPcmFileAbsolutePath(context, fileName));
                }
                //清除
                fileNameList.clear();
...
//                mergePCMFilesToWAVFile(filePaths);
     ...
     * @param listener 音频流的监听
     */
    private void writeDataTOFile(Context context, AudioRecordStreamListener listener) {
        ...
        try {
            ...
            File file = new File(AudioFileUtil.getPcmFileAbsolutePath(context, currentFileName));
            if (file.exists()) {
                file.delete();
            }
     ...
     * @param filePaths
     */
    private void mergePCMFilesToWAVFile(Context context, final List<String> filePaths) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                if (AudioPcmToWav.mergePCMFilesToWAVFile(filePaths, AudioFileUtil.getWavFileAbsolutePath(context, fileName))) {
                    //操作成功
                } else {
                    //操作失败
